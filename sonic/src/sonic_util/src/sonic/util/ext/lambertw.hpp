...
namespace lambertw_detail {
inline constexpr double e_constant = 2.71828182845904523536028747135266250;
inline constexpr double e_inverse = 1.0 / e_constant;
} // namespace lambertw_detail
...
inline double LambertW0(const double z) {
  ...
  static double a[12];
  static double b[12];
  constexpr double e_val = lambertw_detail::e_constant;
  constexpr double e_inv = lambertw_detail::e_inverse;
...
public:
  template <int order> static Float BranchPointExpansion(const Float x) {
    return Horner<Float, BranchPoint, order>::Eval(eSign * std::sqrt(2 * (Float(lambertw_detail::e_constant) * x + 1)));
  }

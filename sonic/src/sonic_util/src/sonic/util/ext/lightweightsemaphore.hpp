...
#ifndef MOODYCAMEL_NOEXCEPT_CTOR
#define MOODYCAMEL_NOEXCEPT_CTOR(type, valueType, expr) noexcept(expr)
#endif

#ifndef MOODYCAMEL_NOEXCEPT_ASSIGN
...
__declspec(dllimport) void* __stdcall CreateSemaphoreW(
    _SECURITY_ATTRIBUTES* lpSemaphoreAttributes, long lInitialCount, long lMaximumCount, const wchar_t* lpName
);
  ...
  bool waitWithPartialSpinning(std::int64_t timeout_usecs = -1) {
    ...
    while (--spin >= 0) {
      ...
      if ((oldCount > 0) && m_count.compare_exchange_strong(
                                oldCount, oldCount - 1, std::memory_order_acquire, std::memory_order_relaxed
                            )) {
        return true;
      }
    ...
    while (true) {
      ...
      if (oldCount < 0 && m_count.compare_exchange_strong(
                              oldCount, oldCount + 1, std::memory_order_relaxed, std::memory_order_relaxed
                          )) {
        return false;
      }
    }

...
ORam::ORam(int starting_size_power_of_two, size_t val_len)
    : capacity_(1UL << starting_size_power_of_two), val_len_(val_len),
...
bool IsPowerOfTwo(size_t x) { return !(x & (x - 1)); }

namespace {

void ObliviousSwapBlock(PORamBlock &a, PORamBlock &b, bool cond,
                        size_t val_len) {
...
void ObliviousMergeHalves(std::vector<PORamBlock> &A, size_t start,
                          size_t length, size_t val_len) {
...
void OCompact(std::vector<PORamBlock> &A, size_t start, size_t length,
              size_t val_len) {
  if (length <= 1)
    return;
...
Block ORam::ReadAndRemove(Key k, crypto::Key enc_key) {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    phys_k = sn::obliv::ct_select<uint64_t>(
        phys_k, 1, is_real); // Dummy access to slot 1 if not found
    auto bl = sub_orams_[i]->ReadAndRemove(0, phys_k, enc_key, is_real);

    res.key_ =
        sn::obliv::ct_select<Key>(k, res.key_, is_real && (bl.meta_.key_ != 0));
    if (bl.val_) {
      sn::obliv::ct_select_array(res.val_.get(), bl.val_.get(), res.val_.get(),
                                 val_len_, is_real);
    }
  }
...
void ORam::Insert(Key k, Val v, crypto::Key enc_key) {
  ...
  for (uint64_t i = 1; i <= cap_L; ++i) {
    ...
    log_map_[1][i] =
        sn::obliv::ct_select<uint64_t>(k, log_map_[1][i], select_this);
  }
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    static_path_oram::Block b(static_cast<uint32_t>(0),
                              static_cast<uint32_t>(target_phys_k));
    if (v) {
...
void ORam::ExecuteBatch(std::vector<BatchOperation> &batch, crypto::Key enc_key,
  ...
  struct BatchSwapHook {
    ...
    void operator()(OblivElem *a, OblivElem *b, bool cond) const {
      ...
      if (val_len > 0) {
        sn::obliv::ct_swap_array(batch_ref[idx_a].val.get(),
                                 batch_ref[idx_b].val.get(), val_len, cond);
      }
    }
  };
  ...
  sn::sortshuffle::ser::bitonic::detail::bitonic_sort_impl(
      elems.data(), B, key_ext, comp1, hook);

  // After bitonic sort, batch is co-sorted with elems via the hook.
  // Reset seq to current position so batch[elems[i].seq] == batch[i] stays
  // correct even after the later std::sort reorders elems without touching
  // batch.
  for (size_t i = 0; i < B; ++i) {
    elems[i].seq = static_cast<uint32_t>(i);
  }
  ...
  // MUST happen after bitonic sort because the hook only co-sorts
  // .type/.key/.val, not .phys_k/.sub_oram_idx. Routing here ensures alignment
  // with the sorted batch.
  for (auto &op : batch) {
    op.sub_oram_idx = -1;
    op.phys_k = 0;
  }
  ...
  if (cap_S > 0) {
    for (uint64_t i = 1; i <= cap_S; ++i) {
      ...
      for (auto &op : batch) {
        bool match = (log_k != 0) &&
                     sn::obliv::ct_eq(static_cast<uint64_t>(op.key), log_k);
        op.sub_oram_idx =
            sn::obliv::ct_select<int8_t>(0, op.sub_oram_idx, match);
    ...
    for (uint64_t i = 1; i <= cap_L; ++i) {
      ...
      for (auto &op : batch) {
        bool match = (log_k != 0) &&
                     sn::obliv::ct_eq(static_cast<uint64_t>(op.key), log_k);
        op.sub_oram_idx =
            sn::obliv::ct_select<int8_t>(1, op.sub_oram_idx, match);
        op.phys_k = sn::obliv::ct_select<uint64_t>(i, op.phys_k, match);
      }
    }
  }
  ...
  // Pass 1: Forward Scan
  // Propagates the latest payload to Search operations, and tracks if the key
  // was deleted.
  ...
  for (size_t i = 0; i < B; ++i) {
    bool start_of_key =
        (i == 0) || !sn::obliv::ct_eq(elems[i].key, elems[i - 1].key);
    has_payload = sn::obliv::ct_select(false, has_payload, start_of_key);
    is_deleted = sn::obliv::ct_select(false, is_deleted, start_of_key);

    bool is_insert = sn::obliv::ct_eq(elems[i].op_type,
                                      static_cast<uint8_t>(OpType::Insert));
    bool is_update = sn::obliv::ct_eq(elems[i].op_type,
                                      static_cast<uint8_t>(OpType::Update));
    bool is_delete = sn::obliv::ct_eq(elems[i].op_type,
                                      static_cast<uint8_t>(OpType::Delete));
    bool is_search = sn::obliv::ct_eq(elems[i].op_type,
                                      static_cast<uint8_t>(OpType::Search));

    bool is_write = is_insert | is_update;

    if (val_len_ > 0) {
      sn::obliv::ct_select_array(current_payload.data(), batch[i].val.get(),
                                 current_payload.data(), val_len_, is_write);
    }
    ...
    if (val_len_ > 0) {
      if (!batch[i].result.val_) {
        ...
        std::fill(batch[i].result.val_.get(),
                  batch[i].result.val_.get() + val_len_, 0);
      }
      sn::obliv::ct_select_array(
          batch[i].result.val_.get(), current_payload.data(),
          batch[i].result.val_.get(), val_len_, forward_to_search);
    }
    batch[i].result.key_ = sn::obliv::ct_select<uint64_t>(
        1, batch[i].result.key_, forward_to_search);
    // If a search receives a forwarded payload, or if it searches a deleted
    // key, it becomes dummy. If an update targets a deleted key, it is a no-op
    // and also becomes dummy.
    bool search_becomes_dummy =
        (is_search & has_payload) | (is_search & is_deleted);
    bool update_becomes_dummy = is_update & is_deleted;
    sn::obliv::ct_set_ref(elems[i].is_dummy, true,
                          search_becomes_dummy | update_becomes_dummy);
  }
  ...
  for (int64_t i = B - 1; i >= 0; --i) {
    bool end_of_key = (static_cast<size_t>(i) == B - 1) ||
                      !sn::obliv::ct_eq(elems[i].key, elems[i + 1].key);
    has_future_write =
        sn::obliv::ct_select(false, has_future_write, end_of_key);
    has_future_delete =
        sn::obliv::ct_select(false, has_future_delete, end_of_key);
    has_future_access =
        sn::obliv::ct_select(false, has_future_access, end_of_key);
    ...
    // If this is the FIRST write we see going backwards, it captures the
    // payload
    bool captures_payload =
        (is_insert | is_update) & !has_future_write & !has_future_delete;
    if (val_len_ > 0) {
      sn::obliv::ct_select_array(backward_payload.data(), batch[i].val.get(),
                                 backward_payload.data(), val_len_,
                                 captures_payload);
    }

    // If there is a future write or delete, this operation might be
    // overshadowed and become dummy
    bool overshadowed =
        (is_insert | is_update) & (has_future_write | has_future_delete);
    // Deletes are ONLY overshadowed by future deletes, NEVER by future writes
    // (which allocate new slots)
    ...
    // Exception: If this is an Insert, and there are future Updates, the Insert
    // MUST remain real (to be processed by InsertBatch and increment real_I).
    ...
    overshadowed =
        sn::obliv::ct_select(false, overshadowed, is_dominant_insert);

    if (val_len_ > 0) {
      sn::obliv::ct_select_array(batch[i].val.get(), backward_payload.data(),
                                 batch[i].val.get(), val_len_,
                                 is_dominant_insert);
    }

    sn::obliv::ct_set_ref(elems[i].is_dummy, true, overshadowed);

    // If this operation was an Insert that remained real, it absorbs all future
    // writes, so we set has_future_write = true (which it already is) but it
    // effectively overshadows PAST writes.
    has_future_write = has_future_write | is_insert | is_update;
    has_future_delete = sn::obliv::ct_select(
        false, has_future_delete, is_insert); // Insert cancels future delete
  ...
  // To fix this, we need a forward scan to track if a Delete is deleting an
  // Insert from the SAME batch.
  ...
  for (size_t i = 0; i < B; ++i) {
    ...
    // If we see a Delete and we've seen an Insert for this key in the same
    // batch, the Delete becomes dummy
  ...
  for (size_t i = 0; i < B; ++i) {
    ...
    real_DS +=
        sn::obliv::ct_select<size_t>(1, 0, is_real && is_delete && (idx == 0));
    real_DL +=
        sn::obliv::ct_select<size_t>(1, 0, is_real && is_delete && (idx == 1));
  }
  ...
  for (size_t i = 0; i < B; ++i) {
    ...
    for (uint64_t j = 1; j <= cap_L; ++j) {
      ...
      assigned_slot =
          sn::obliv::ct_select<uint64_t>(j, assigned_slot, select_this);
      log_map_[1][j] = sn::obliv::ct_select<uint64_t>(
          elems[i].key, log_map_[1][j], select_this);
    }

    batch[orig_idx].sub_oram_idx = sn::obliv::ct_select<int8_t>(
        1, batch[orig_idx].sub_oram_idx, needs_slot);
    batch[orig_idx].phys_k = sn::obliv::ct_select<uint64_t>(
        assigned_slot, batch[orig_idx].phys_k, needs_slot);
  }

  // Phase 3: O-Sort (Group by OpType)
  auto comp3 = [](const OblivElem &a, const OblivElem &b) {
    bool a_is_ins =
        sn::obliv::ct_eq(a.op_type, static_cast<uint8_t>(OpType::Insert));
    bool b_is_ins =
        sn::obliv::ct_eq(b.op_type, static_cast<uint8_t>(OpType::Insert));
  ...
  sn::sortshuffle::ser::bitonic::detail::bitonic_sort_impl(
      elems.data(), B, key_ext, comp3, hook);
  ...
  for (size_t i = original_accesses; i < B; ++i) {
    ...
    if (batch[orig_idx].val) {
      b.val_ = std::make_unique<uint8_t[]>(val_len_);
      std::copy(batch[orig_idx].val.get(), batch[orig_idx].val.get() + val_len_,
                b.val_.get());
    }
  ...
  for (size_t i = 0; i < original_accesses; ++i) {
    ...
    bool is_search =
        sn::obliv::ct_eq(op_type, static_cast<uint8_t>(OpType::Search));
    bool is_delete =
        sn::obliv::ct_eq(op_type, static_cast<uint8_t>(OpType::Delete));
    bool is_update =
        sn::obliv::ct_eq(op_type, static_cast<uint8_t>(OpType::Update));
  ...
  if (sub_orams_[0]) {
    auto read_results =
        sub_orams_[0]->ReadBatch(small_ops, enc_key, steady_state);
    for (size_t i = 0; i < original_accesses; ++i) {
      if (small_ops[i].is_real) {
        ...
        if (read_results[i].val_) {
          ...
          std::copy(read_results[i].val_.get(),
                    read_results[i].val_.get() + val_len_,
                    batch[orig_idx].result.val_.get());
        }
      }
    }
  }
  if (sub_orams_[1]) {
    auto read_results =
        sub_orams_[1]->ReadBatch(large_ops, enc_key, steady_state);
    for (size_t i = 0; i < original_accesses; ++i) {
  ...
  // Phase 2.5: Post-ReadBatch backward scan to propagate old values to dummy
  // Searches
  ...
  for (int64_t i = original_accesses - 1; i >= 0; --i) {
    bool end_of_key = (i == static_cast<int64_t>(original_accesses) - 1) ||
                      !sn::obliv::ct_eq(elems[i].key, elems[i + 1].key);
    ...
    if (val_len_ > 0) {
      sn::obliv::ct_select_array(old_payload.data(),
                                 batch[orig_idx].result.val_.get(),
                                 old_payload.data(), val_len_, is_real);
    }
    old_key = sn::obliv::ct_select<uint64_t>(batch[orig_idx].result.key_,
                                             old_key, is_real);
    has_old_payload = has_old_payload | is_real;

    bool received_forward =
        sn::obliv::ct_eq<uint64_t>(batch[orig_idx].result.key_, 1);
    bool needs_old_payload =
        is_search & !is_real & has_old_payload & !received_forward;
    ...
    if (val_len_ > 0) {
      ...
      sn::obliv::ct_select_array(
          batch[orig_idx].result.val_.get(), old_payload.data(),
          batch[orig_idx].result.val_.get(), val_len_, needs_old_payload);
    }

    uint64_t final_key = sn::obliv::ct_select<uint64_t>(batch[orig_idx].key, 0,
                                                        received_forward);
    final_key =
        sn::obliv::ct_select<uint64_t>(old_key, final_key, needs_old_payload);
    final_key = sn::obliv::ct_select<uint64_t>(batch[orig_idx].result.key_,
                                               final_key, is_real & is_search);
  ...
  if (sub_orams_[1]) {
    ...
    for (auto &b : inserts) {
      ...
      static_path_oram::Block new_b(static_cast<static_path_oram::Pos>(phys_k),
                                    static_cast<static_path_oram::Key>(phys_k));
      if (b.val_) {
  ...
  if (target_small <= 0) {
    ...
    int64_t tightened_limit =
        std::min(static_cast<int64_t>(real_I), total_deletes - (y / 2));
  ...
  if (sub_orams_[0] && sub_orams_[1] && T > 0) {
    ...
    if (k_transfer > 0) {
      extracted =
          sub_orams_[0]->ObliviousExtractValidKeys(k_transfer, T, no_filter);
    } else if (k_transfer < 0) {
      extracted =
          sub_orams_[1]->ObliviousExtractValidKeys(-k_transfer, T, no_filter);
    } else {
    ...
    if (k_transfer > 0)
      Buffer = sub_orams_[0]->ReadAndRemoveBatch(keys_to_read, enc_key);
    else if (k_transfer < 0)
      Buffer = sub_orams_[1]->ReadAndRemoveBatch(keys_to_read, enc_key);
    ...
    if (k_transfer != 0) {
      for (int64_t i = T; i < T_pow2; ++i) {
        ...
        if (val_len_ > 0) {
          ...
          std::fill(Buffer.back().val_.get(),
                    Buffer.back().val_.get() + val_len_, 0);
        }
      }
  ...
  if (scale_up) {
    ...
    int64_t excess =
        std::max<int64_t>(0, static_cast<int64_t>(capacity_) - old_y);
    ...
    if (k_sec > 0) {
      auto no_filter = [](Key phys_k) { return true; };
      std::vector<Key> extracted =
          sub_orams_[0]->ObliviousExtractValidKeys(k_sec, k_sec, no_filter);
      ...
      std::vector<static_path_oram::Block> Buffer =
          sub_orams_[0]->ReadAndRemoveBatch(keys_to_read, enc_key);
      ...
      for (int64_t i = 0; i < k_sec; ++i) {
        ...
        for (uint64_t j = 1; j <= static_cast<uint64_t>(old_y); ++j) {
          ...
          log_map_[0][j] =
              sn::obliv::ct_select<uint64_t>(0, log_map_[0][j], match);
        }
        ...
        for (uint64_t j = 1; j <= static_cast<uint64_t>(2 * old_y); ++j) {
          ...
          new_phys_k = sn::obliv::ct_select(j, new_phys_k, select_this);
          log_map_[1][j] =
              sn::obliv::ct_select(log_k, log_map_[1][j], select_this);
        }
    ...
    int64_t deficit =
        std::max<int64_t>(0, old_x - static_cast<int64_t>(capacity_));
    ...
    if (k_sec > 0) {
      auto no_filter = [](Key phys_k) { return true; };
      std::vector<Key> extracted =
          sub_orams_[1]->ObliviousExtractValidKeys(k_sec, k_sec, no_filter);
      ...
      std::vector<static_path_oram::Block> Buffer =
          sub_orams_[1]->ReadAndRemoveBatch(keys_to_read, enc_key);
      ...
      for (int64_t i = 0; i < k_sec; ++i) {
        ...
        for (uint64_t j = 1; j <= static_cast<uint64_t>(old_x); ++j) {
          ...
          log_map_[1][j] =
              sn::obliv::ct_select<uint64_t>(0, log_map_[1][j], match);
        }
        ...
        for (uint64_t j = 1; j <= static_cast<uint64_t>(old_x / 2); ++j) {
          ...
          new_phys_k = sn::obliv::ct_select(j, new_phys_k, select_this);
          log_map_[0][j] =
              sn::obliv::ct_select(log_k, log_map_[0][j], select_this);
        }

...
class SonicORamAdapter {
 public:
  ...
  std::vector<static_path_oram::Block> ReadAndRemoveBatch(const std::vector<AccessOp>& ops, crypto::Key enc_key, bool steady_state = true);
  void InsertBatch(std::vector<static_path_oram::Block>& blocks, crypto::Key enc_key, bool steady_state = true, bool all_new = false);

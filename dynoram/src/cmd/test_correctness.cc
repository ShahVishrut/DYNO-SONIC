...
void TestOMap() {
  ...
  try {
    ...
    for (int i = 1; i <= 10; ++i) {
      ...
      if (res.get()[0] != i * 5) {
        std::cerr << "OMap Read mismatch for key " << i << "! Expected "
                  << i * 5 << " but got " << (int)res.get()[0] << "\n";
      }
...
void TestOHeap() {
  ...
  for (int exp : expected) {
    ...
    assert(res.meta_.key_ == exp &&
           "Extracted key should match expected sorted order!");
    assert(res.val_.get()[0] == exp &&
           "Extracted value should match expected value!");
  }
    ...
    for (int i = 1; i <= 1024; ++i) {
      ...
      if (i % 64 == 0) {
        ...
        if (res.val_.get()[0] != read_key % 255) {
          std::cerr << "ORam Read mismatch! Expected " << (read_key % 255)
                    << ", got " << (int)res.val_.get()[0] << "\n";
        }
        assert(res.val_.get()[0] == read_key % 255 &&
               "Value should match what was inserted!");
      }
    }
  } catch (const std::exception &e) {
    std::cerr << "ORam Test Failed! Exception: " << e.what()
              << " at Size=" << oram->Size()
...
void TestORamBatch() {
  ...
  std::vector<dyno::dynamic_stepping_path_oram::ORam::BatchOperation>
      init_batch;
  ...
  {
    dyno::dynamic_stepping_path_oram::ORam::BatchOperation
        op1; // Search 5 (should be 50)
  ...
  // Verify results of batch1 in-place (since ExecuteBatch modifies the passed
  // structs)
  ...
  std::vector<dyno::dynamic_stepping_path_oram::ORam::BatchOperation> batch2;
  {
    ...
    dyno::dynamic_stepping_path_oram::ORam::BatchOperation
        op2; // Search 12 (Immediately after insert, should return 120)
    ...
    dyno::dynamic_stepping_path_oram::ORam::BatchOperation
        op4; // Delete 13 (Immediately after insert, cancels out)
  ...
  assert(batch2[1].val.get()[0] == 120 &&
         "Search 12 should collapse with Insert 12 and return 120!");

  // Verify ORAM State post-batches
  // Key 6 should be deleted
  auto res6 = oram->Read(6, enc_key);
  assert(res6.val_ == nullptr ||
         res6.val_.get()[0] == 0 && "Key 6 should have been deleted!");

  // Key 11 should be inserted
  auto res11 = oram->Read(11, enc_key);
  assert(res11.val_ != nullptr && res11.val_.get()[0] == 110 &&
         "Key 11 should be present!");

  // Key 12 should be inserted
  auto res12 = oram->Read(12, enc_key);
  assert(res12.val_ != nullptr && res12.val_.get()[0] == 120 &&
         "Key 12 should be present!");

  // Key 13 should NOT be present (Insert + Delete cancelled out)
  auto res13 = oram->Read(13, enc_key);
  assert((res13.val_ == nullptr || res13.val_.get()[0] == 0) &&
...
void TestORamComprehensiveMixedWorkload() {
  std::cout
      << "Testing DYNO+SONIC Comprehensive Mixed Workload Correctness...\n";
  auto enc_key = GenerateKey();

  auto oram = std::make_unique<dyno::dynamic_stepping_path_oram::ORam>(
      10, 8); // Capacity 1024, val_len 8
  ...
  std::vector<int> batch_sizes = {2048, 512, 1024, 128, 4096};
  
  for (size_t b = 0; b < batch_sizes.size(); ++b) {
    int current_batch_size = batch_sizes[b];
    std::cout << "  Running mixed batch " << b + 1 << "/" << batch_sizes.size() << " of size " << current_batch_size << "...\n";
    
    // Change seed per batch slightly for variety, but keep deterministic
    rng.seed(1337 + b); 
    ...
    for (int i = 0; i < current_batch_size; ++i) {
      ...
      if (op_rand < 15) {
        ...
        std::uniform_int_distribution<uint64_t> dist_key_dynamic(1, next_new_key - 1);
        op.key = dist_key_dynamic(rng);
    ...
    // Verify State after this batch
    std::cout << "  Verifying State after batch " << b + 1 << "...\n";
      ...
      if (found_in_shadow) {
        if (!found_in_oram) {
          ...
          std::cerr << "Mismatch at Key " << k << ": ORAM=" << oram_val
                    << ", Shadow=" << shadow_state[k] << "\n";
          assert(false);
        }
      } else {
        // Because ORAM returns zeroed bytes if deleted/not found
        if (found_in_oram && oram_val != 0) {
          std::cerr << "Mismatch at Key " << k << ": Exists in ORAM (" << oram_val
                    << ") but NOT in shadow map!\n";
          assert(false);
        }
      }
    }
...
void TestORamDeterministicScale() {
  ...
  for (int i = 1; i <= 3; ++i) {
    auto res = oram->Read(i, enc_key);
    std::cout << "[DEBUG] Read Key " << i << " returned val_ is null: " << (res.val_ == nullptr)
              << ", val: " << (res.val_ != nullptr ? (int)res.val_.get()[0] : -1) 
              << ", key: " << res.key_ << "\n";
  ...
  assert((res516.val_ == nullptr || res516.val_.get()[0] == 0) &&
         "Key 516 never existed!");

  // 4. Edge-case collapses
  std::vector<dyno::dynamic_stepping_path_oram::ORam::BatchOperation> batch4;
  auto make_op = [](dyno::dynamic_stepping_path_oram::ORam::OpType type,
                    uint64_t key, uint8_t val) {
    dyno::dynamic_stepping_path_oram::ORam::BatchOperation op;
    op.type = type;
    op.key = key;
    if (type == dyno::dynamic_stepping_path_oram::ORam::OpType::Insert ||
        type == dyno::dynamic_stepping_path_oram::ORam::OpType::Update) {
      op.val = std::make_unique<uint8_t[]>(8);
  ...
  batch4.push_back(
      make_op(dyno::dynamic_stepping_path_oram::ORam::OpType::Delete, 2000,
              0)); // Non-existent
  batch4.push_back(
      make_op(dyno::dynamic_stepping_path_oram::ORam::OpType::Insert, 1540,
              100)); // Insert 1540
  batch4.push_back(
      make_op(dyno::dynamic_stepping_path_oram::ORam::OpType::Update, 1540,
              200)); // Update 1540
  batch4.push_back(
      make_op(dyno::dynamic_stepping_path_oram::ORam::OpType::Search, 1540,
              0)); // Search 1540

  auto add_op = [&batch4,
                 &make_op](dyno::dynamic_stepping_path_oram::ORam::OpType type,
                           uint64_t key, uint8_t val = 0) {
  ...
  assert(batch4[3].val.get()[0] ==
         200); // the search should find the updated value
  ...
  assert(batch4[6].val != nullptr && batch4[6].val.get()[0] == 11 &&
         "Key 1000 Search should return 11!");
  assert((batch4[9].val == nullptr || batch4[9].val.get()[0] == 0) &&
         "Key 1001 Search should return null!");
  assert(batch4[11].val != nullptr && batch4[11].val.get()[0] == 10 &&
         "Key 1002 Search should return 10!");
  assert(batch4[14].val != nullptr && batch4[14].val.get()[0] == 11 &&
         "Key 1003 Search should return 11!");
  assert(batch4[17].val != nullptr && batch4[17].val.get()[0] == 10 &&
         "Key 1004 Search should return 10!");

  // Check final state in ORAM
  auto res1000 = oram->Read(1000, enc_key);
  assert(res1000.val_ != nullptr && res1000.val_.get()[0] == 11);
  auto res1001 = oram->Read(1001, enc_key);
  assert((res1001.val_ == nullptr || res1001.val_.get()[0] == 0));
  auto res1002 = oram->Read(1002, enc_key);
  assert(res1002.val_ != nullptr && res1002.val_.get()[0] == 11);
  auto res1003 = oram->Read(1003, enc_key);
  assert(res1003.val_ != nullptr && res1003.val_.get()[0] == 11);
  auto res1004 = oram->Read(1004, enc_key);
  assert(res1004.val_ != nullptr && res1004.val_.get()[0] == 10);
  auto res1005 = oram->Read(1005, enc_key);
  assert(res1005.val_ != nullptr && res1005.val_.get()[0] == 11);

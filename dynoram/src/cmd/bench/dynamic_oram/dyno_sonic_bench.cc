...
void RunDynoWorkload(ORam& oram, dyno::crypto::Key enc_key, const std::string& test_name, int num_ops, int mix_type, bool steady_state = true) {
    ...
    for (int i = 0; i < num_ops; ++i) {
        ...
        if (type_to_use == 0) {
            ...
            op.val = std::make_unique<uint8_t[]>(48); // Assuming val_len = 48
            std::memset(op.val.get(), 1, 48);
        } else if (type_to_use == 2) {
            op.type = ORam::OpType::Insert;
            op.val = std::make_unique<uint8_t[]>(48);
            std::memset(op.val.get(), 2, 48);
...
void TestDynoContinuousScaling(dyno::crypto::Key enc_key, size_t batch_size) {
    ...
    ORam scaling_oram(start_pow2, 48); 
    std::mt19937 rng(42);
...
int main(int argc, char** argv) {
    ...
    ORam oram(23, 48); // 2^23 capacity, 48 byte val_len

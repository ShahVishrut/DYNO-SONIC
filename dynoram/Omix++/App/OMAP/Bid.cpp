...
Bid::Bid() { std::fill(id.begin(), id.end(), 0); }
...
bool Bid::operator!=(const Bid rhs) const { return !CTeq(CTcmp(id, rhs), 0); }

bool Bid::operator<(const Bid& rhs) const { return CTeq(CTcmp(id, rhs), -1); }
...
void Bid::setToZero() { std::fill(id.begin(), id.end(), 0); }

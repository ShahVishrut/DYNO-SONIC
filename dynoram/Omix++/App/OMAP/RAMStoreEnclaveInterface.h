...
void ocall_nwrite_ramStore(size_t blockCount, long long *indexes,
                           const char *blk, size_t len) {
...
void ocall_nwrite_rawRamStore(size_t blockCount, long long *indexes,
                              const char *blk, size_t len) {
  assert(len % blockCount == 0);
  size_t eachSize = len / blockCount;
  for (unsigned int i = 0; i < blockCount; i++) {
...
void ocall_nwrite_ramStore_by_client(vector<long long> *indexes,
...
size_t ocall_nread_ramStore(size_t blockCount, long long *indexes, char *blk,
                            size_t len) {
  assert(len % blockCount == 0);
  size_t resLen = -1;
  for (unsigned int i = 0; i < blockCount; i++) {
...
size_t ocall_nread_rawRamStore(size_t blockCount, size_t begin, char *blk,
                               size_t len) {
  assert(len % blockCount == 0);
  size_t resLen = -1;
...
void ocall_initialize_ramStore(long long begin, long long end, const char *blk,
                               size_t len) {

...
class Node {
 public:
  ...
  static int CTcmp(long long lhs, long long rhs) {
    ...
    int is_less_than =
        (int)-(overflowing_iff_lt >> 127);  // -1 if self < other, 0 otherwise.
    int is_greater_than =
        (int)(overflowing_iff_gt >> 127);  // 1 if self > other, 0 otherwise.
  ...
  static long long conditional_select(long long a, long long b, int choice) {
    ...
    return (~((unsigned long long)choice - one) & a) |
           ((unsigned long long)(choice - one) & b);
  }

  static unsigned long long conditional_select(unsigned long long a,
  ...
  static unsigned int conditional_select(unsigned int a, unsigned int b,
                                         int choice) {
    unsigned int one = 1;
    return (~((unsigned int)choice - one) & a) |
           ((unsigned int)(choice - one) & b);
  }
   ...
   * @return choice = 1 -> a , choice = 0 -> return b
   */
  static unsigned __int128 conditional_select(unsigned __int128 a,
                                              unsigned __int128 b, int choice) {
    unsigned __int128 one = 1;
    return (~((unsigned __int128)choice - one) & a) |
           ((unsigned __int128)(choice - one) & b);
  }
  ...
  static void conditional_assign(Node* a, Node* b, int choice) {
    a->index = Node::conditional_select((long long)b->index,
                                        (long long)a->index, choice);
    a->isDummy = Node::conditional_select(b->isDummy, a->isDummy, choice);
    a->pos =
        Node::conditional_select((long long)b->pos, (long long)a->pos, choice);
    ...
    a->evictionNode =
        Node::conditional_select(b->evictionNode, a->evictionNode, choice);
    ...
    for (int k = 0; k < a->key.id.size(); k++) {
      a->key.id[k] =
          Node::conditional_select(b->key.id[k], a->key.id[k], choice);
    }
    for (int k = 0; k < a->leftID.id.size(); k++) {
      a->leftID.id[k] =
          Node::conditional_select(b->leftID.id[k], a->leftID.id[k], choice);
    }
    for (int k = 0; k < a->rightID.id.size(); k++) {
      a->rightID.id[k] =
          Node::conditional_select(b->rightID.id[k], a->rightID.id[k], choice);
    }
  }
  ...
  static void conditional_swap(Node* a, Node* b, int choice) {
    Node tmp = *b;
    b->index = Node::conditional_select((long long)a->index,
                                        (long long)b->index, choice);
    b->isDummy = Node::conditional_select(a->isDummy, b->isDummy, choice);
    b->pos =
        Node::conditional_select((long long)a->pos, (long long)b->pos, choice);
    for (int k = 0; k < b->value.size(); k++) {
    ...
    b->evictionNode =
        Node::conditional_select(a->evictionNode, b->evictionNode, choice);
    ...
    for (int k = 0; k < b->key.id.size(); k++) {
      b->key.id[k] =
          Node::conditional_select(a->key.id[k], b->key.id[k], choice);
    }
    for (int k = 0; k < b->leftID.id.size(); k++) {
      b->leftID.id[k] =
          Node::conditional_select(a->leftID.id[k], b->leftID.id[k], choice);
    }
    for (int k = 0; k < b->rightID.id.size(); k++) {
      b->rightID.id[k] =
          Node::conditional_select(a->rightID.id[k], b->rightID.id[k], choice);
    }

    a->index = Node::conditional_select((long long)tmp.index,
                                        (long long)a->index, choice);
    a->isDummy = Node::conditional_select(tmp.isDummy, a->isDummy, choice);
    a->pos =
        Node::conditional_select((long long)tmp.pos, (long long)a->pos, choice);
    for (int k = 0; k < b->value.size(); k++) {
    ...
    a->evictionNode =
        Node::conditional_select(tmp.evictionNode, a->evictionNode, choice);
    ...
    for (int k = 0; k < a->key.id.size(); k++) {
      a->key.id[k] =
          Node::conditional_select(tmp.key.id[k], a->key.id[k], choice);
    }
    for (int k = 0; k < a->leftID.id.size(); k++) {
      a->leftID.id[k] =
          Node::conditional_select(tmp.leftID.id[k], a->leftID.id[k], choice);
    }
    for (int k = 0; k < a->rightID.id.size(); k++) {
      a->rightID.id[k] =
          Node::conditional_select(tmp.rightID.id[k], a->rightID.id[k], choice);
    }
  }

  static void conditional_swap(unsigned long long& a, unsigned long long& b,
                               int choice) {
  ...
  static bool CTeq(int a, int b) { return !(a ^ b); }

  static bool CTeq(long long a, long long b) { return !(a ^ b); }

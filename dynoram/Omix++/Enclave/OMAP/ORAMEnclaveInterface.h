...
void ecall_setup_omap_by_client(int max_size, const char* bid,
                                long long rootPos, const char* secretKey) {
  bytes<Key> tmpkey;
...
double ecall_measure_oram_speed(int testSize) { return 0; }
...
double ecall_measure_oram_setup_speed(int testSize) {
  ...
  //    Node* res = oram->ReadWrite(id, dummyNode, readpos, readpos, true,
  //    false);
  //     assert(!res->isDummy);

...
ORAM::ORAM(long long maxSize, bytes<Key> oram_key, bool simulation,
  ...
  storeBlockSize =
      (size_t)(IV + AES::GetCiphertextLength((int)(Z * (blockSize))));
...
void ORAM::InitializeBucketsInBatch() {
  ...
  for (unsigned int j = 0; j <= bucketCount / batchSize; j++) {
    ...
    size_t cipherSize = 0;
    for (int i = 0; i < min((int)(bucketCount - j * batchSize), batchSize);
         i++) {
      Bucket bucket;
      for (int z = 0; z < Z; z++) {
        bucket[z].id = 0;
      ...
      std::memcpy(tmp + i * ciphertext.size(), ciphertext.data(),
                  ciphertext.size());
      cipherSize = ciphertext.size();
    }
    if (min((int)(bucketCount - j * batchSize), batchSize) != 0) {
      ocall_nwrite_ramStore(
          min((int)(bucketCount - j * batchSize), batchSize), indexes.data(),
          (const char*)tmp,
          cipherSize * min((int)(bucketCount - j * batchSize), batchSize));
    }
    delete tmp;
    indexes.clear();
  }
...
void ORAM::WriteBucket(long long index, Bucket bucket) {
  block b = SerialiseBucket(bucket);
  block ciphertext = AES::Encrypt(key, b, clen_size, plaintext_size);
  ocall_write_ramStore(index, (const char*)ciphertext.data(),
...
Bucket ORAM::DeserialiseBucket(block buffer) {
  ...
  for (int z = 0; z < Z; z++) {
    ...
    node->index =
        Node::conditional_select(node->index, nextDummyCounter, !cond);
...
void ORAM::ReadBuckets(vector<long long> indexes) {
  ...
  if (useLocalRamStore) {
    ...
    ocall_nread_ramStore(&readSize, indexes.size(), indexes.data(), tmp,
                         indexes.size() * storeBlockSize);
    for (unsigned int i = 0; i < indexes.size(); i++) {
...
void ORAM::InitializeBuckets(long long strtindex, long long endindex,
                             Bucket bucket) {
  block b = SerialiseBucket(bucket);
  block ciphertext = AES::Encrypt(key, b, clen_size, plaintext_size);
  if (useLocalRamStore) {
    ...
    ocall_initialize_ramStore(strtindex, endindex,
                              (const char*)ciphertext.data(),
                              (size_t)ciphertext.size());
  }
}
...
void ORAM::EvictBuckets() {
  ...
  if (useLocalRamStore) {
    ...
    for (unsigned int j = 0; j <= virtualStorage.size() / 10000; j++) {
      ...
      vector<long long> indexes;
      size_t cipherSize = 0;
      for (int i = 0; i < min((int)(virtualStorage.size() - j * 10000), 10000);
           i++) {
      ...
      if (min((int)(virtualStorage.size() - j * 10000), 10000) != 0) {
        ocall_nwrite_ramStore(
            min((int)(virtualStorage.size() - j * 10000), 10000),
            indexes.data(), (const char*)tmp,
            cipherSize * min((int)(virtualStorage.size() - j * 10000), 10000));
      }
      delete tmp;
      indexes.clear();
    }
...
//            printf("1.index:%llu key:%lld  pos:%llu, isDummy:%d leftKey:%lld
//            left Pos:%llu right key:%lld right pos:%llu\n",
//            stash.nodes[i]->index, stash.nodes[i]->key.getValue(),
//            stash.nodes[i]->pos, stash.nodes[i]->isDummy ? 1 : 0,
//            stash.nodes[i]->leftID.getValue(), stash.nodes[i]->leftPos,
//            stash.nodes[i]->rightID.getValue(), stash.nodes[i]->rightPos);
//    }

Node* ORAM::ReadWrite(Bid bid, Node* inputnode, unsigned long long lastLeaf,
                      unsigned long long newLeaf, bool isRead, bool isDummy,
                      bool isIncRead) {
  if (bid == 0) {
  ...
  unsigned long long fetchPos =
      Node::conditional_select(newPos, lastLeaf, isDummy);
  ...
  if (isIncomepleteRead) {
    nodesList.insert(nodesList.end(), incStash.nodes.begin(),
                     incStash.nodes.end());
  }
...
Node* ORAM::ReadWriteTest(Bid bid, Node* inputnode, unsigned long long lastLeaf,
                          unsigned long long newLeaf, bool isRead, bool isDummy,
                          bool isIncRead) {
  if (bid == 0) {
    printf("bid is 0 dummy is:%d\n", isDummy ? 1 : 0);
...
Node* ORAM::ReadWrite(Bid bid, unsigned long long lastLeaf,
                      unsigned long long newLeaf, bool isDummy,
                      unsigned long long newChildPos, Bid targetNode) {
  if (bid == 0) {
    printf("bid is 0 dummy is:%d\n", isDummy ? 1 : 0);
    throw runtime_error("Node id is not set");
  }
  ...
  for (Node* node : stash.nodes) {
    ...
    node->leftPos = Node::conditional_select(newChildPos, node->leftPos,
                                             !isDummy && match && leftChild);
    node->rightPos = Node::conditional_select(newChildPos, node->rightPos,
                                              !isDummy && match && rightChild);

    if (!isDummy && match) {
      // printf("previous pos:%lld new pos:%lld\n",lastLeaf,newLeaf);
      // printf("in read and set-node:%d:%d:%d:%d:%d:%d:%d\n",
      // node->key.getValue(), node->height, node->pos, node->leftID.getValue(),
      // node->leftPos, node->rightID.getValue(), node->rightPos); printf("in
      // read and set-res:%d:%d:%d:%d:%d:%d:%d\n", res->key.getValue(),
      // res->height, res->pos, res->leftID.getValue(), res->leftPos,
      // res->rightID.getValue(), res->rightPos);
    }
  }
...
Node* ORAM::ReadWrite(Bid bid, Node* inputnode, unsigned long long lastLeaf,
                      unsigned long long newLeaf, bool isRead, bool isDummy,
                      std::array<byte_t, 16> value, bool overwrite,
                      bool isIncRead) {
  if (bid == 0) {
    printf("bid is 0 dummy is:%d\n", isDummy ? 1 : 0);
    throw runtime_error("Node id is not set");
  ...
  for (Node* node : nodesList) {
    ...
    node->pos = Node::conditional_select(newLeaf, node->pos, !isDummy && match);
    for (int k = 0; k < res->value.size(); k++) {
      node->value[k] = Node::conditional_select(value[k], node->value[k],
                                                !isDummy && match && overwrite);
    }
    bool choice = !isDummy && match && isRead && !node->isDummy;
    res->index = Node::conditional_select((long long)node->index,
                                          (long long)res->index, choice);
...
void ORAM::evict(bool evictBucketsForORAM) {
  ...
  for (Node* node : stash.nodes) {
    long long xorVal = 0;
    xorVal = Node::conditional_select((unsigned long long)0,
                                      node->pos ^ currentLeaf, node->isDummy);
    long long indx = 0;

    indx = (long long)floor(log2(Node::conditional_select(
        xorVal, (long long)1, Node::CTcmp(xorVal, 0))));
    ...
    for (long long i = 0; i < firstIndexes.size(); i++) {
      ...
      node->evictionNode = Node::conditional_select(
          firstIndexes[indx], node->evictionNode, !node->isDummy && choice);
    }
  }
  ...
  for (unsigned long long i = 0; i < stash.nodes.size(); i++) {
    ...
    bool firstCond =
        (!Node::CTeq(Node::CTcmp(counter - (depth - level) * Z, Z), -1));
    bool secondCond =
        Node::CTeq(Node::CTcmp(curNode->evictionNode, currentID), 0);
    bool thirdCond =
        (!Node::CTeq(Node::CTcmp(counter, Z * depth), -1)) || curNode->isDummy;
    bool fourthCond = Node::CTeq(curNode->evictionNode, currentID);

    long long tmpEvictionNode =
        GetNodeOnPath(currentLeaf, depth - (int)floor(counter / Z));
    long long tmpcurrentID = GetNodeOnPath(currentLeaf, level - 1);
    curNode->evictionNode =
        Node::conditional_select((long long)-1, curNode->evictionNode,
                                 firstCond && secondCond && thirdCond);
    curNode->evictionNode =
        Node::conditional_select(tmpEvictionNode, curNode->evictionNode,
                                 firstCond && secondCond && !thirdCond);
    counter = Node::conditional_select(counter + 1, counter,
                                       firstCond && secondCond && !thirdCond);
    counter = Node::conditional_select(counter + 1, counter,
                                       !firstCond && fourthCond);
    level =
        Node::conditional_select(level - 1, level, firstCond && !secondCond);
    i = Node::conditional_select(i - 1, i, firstCond && !secondCond);
    currentID = Node::conditional_select(tmpcurrentID, currentID,
                                         firstCond && !secondCond);

    //        if (firstCond) {
    //            if (secondCond) {
    //                if (thirdCond) {
    //                    long long tmpEvictionNode = GetNodeOnPath(currentLeaf,
    //                    depth - (int) floor(counter / Z));
  ...
  for (int i = 0; i < (depth + 1) * Z; i++) {
    ...
    curBlock.id = Node::conditional_select((unsigned long long)0,
                                           cureNode->index, cureNode->isDummy);
    for (int k = 0; k < tmp.size(); k++) {
      curBlock.data[k] =
          Node::conditional_select(curBlock.data[k], tmp[k], cureNode->isDummy);
    }
    delete cureNode;
    j++;
  ...
  stash.nodes.erase(stash.nodes.begin(),
                    stash.nodes.begin() + ((depth + 1) * Z));

  for (unsigned int i = PERMANENT_STASH_SIZE; i < stash.nodes.size(); i++) {
    delete stash.nodes[i];
  }
  stash.nodes.erase(stash.nodes.begin() + PERMANENT_STASH_SIZE,
                    stash.nodes.end());
...
ORAM::ORAM(long long maxSize, bytes<Key> oram_key, vector<Node*>* nodes,
  ...
  for (unsigned int j = 0; j <= indexes.size() / 10000; j++) {
    ...
    if (min((int)(indexes.size() - j * 10000), 10000) != 0) {
      ocall_nwrite_ramStore(
          min((int)(indexes.size() - j * 10000), 10000),
          indexes.data() + j * 10000, (const char*)tmp,
          cipherSize * min((int)(indexes.size() - j * 10000), 10000));
    }
    delete tmp;
  }

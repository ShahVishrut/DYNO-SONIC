    ...
    while (d > 0) {
      ...
      while (i < len - d) {
        if ((i & p) == r) {
          ...
          if (i != j) {
            int node_cmp =
                Node::CTcmp((*data)[j]->evictionNode, (*data)[i]->evictionNode);
            int dummy_blocks_last =
                Node::CTcmp((*data)[i]->isDummy, (*data)[j]->isDummy);
            int same_nodes = Node::CTeq(node_cmp, 0);
            bool cond = Node::CTeq(Node::conditional_select(
                                       dummy_blocks_last, node_cmp, same_nodes),
                                   -1);
...
void ObliviousOperations::bitonic_sort(vector<Node*>* nodes, int low, int n,
                                       int dir) {
  if (n > 1) {
    int middle = n / 2;
...
void ObliviousOperations::bitonic_merge(vector<Node*>* nodes, int low, int n,
                                        int dir) {
  if (n > 1) {
...
void ObliviousOperations::compare_and_swap(Node* item_i, Node* item_j,
                                           int dir) {

...
static void error(const char *msg) { throw msg; }

int AES::EncryptBytes(bytes<Key> key, bytes<IV> iv, byte_t *plaintext,
                      size_t plen, byte_t *ciphertext) {
...
int AES::DecryptBytes(bytes<Key> key, bytes<IV> iv, byte_t *ciphertext,
                      size_t clen, byte_t *plaintext) {
  EVP_CIPHER_CTX *ctx = EVP_CIPHER_CTX_new();
...
block AES::EncryptBlock(bytes<Key> key, bytes<IV> iv, block plaintext,
                        size_t clen_size, size_t plaintext_size) {
...
block AES::DecryptBlock(bytes<Key> key, bytes<IV> iv, block ciphertext,
  ...
  int plen =
      DecryptBytes(key, iv, ciphertext.data(), clen_size, plaintext.data());
...
block AES::Encrypt(bytes<Key> key, block plaintext, size_t clen_size,

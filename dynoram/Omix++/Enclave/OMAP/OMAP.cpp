...
OMAP::OMAP(int maxSize, bytes<Key> secretKey, map<Bid, string>* pairs,
           map<unsigned long long, unsigned long long>* permutation) {
  treeHandler =
      new AVLTree(maxSize, secretKey, rootKey, rootPos, pairs, permutation);
}
...
void OMAP::insert(Bid omapKey, string value) {
  ...
  if (rootKey == 0) {
    rootKey =
        treeHandler->insert(0, rootPos, omapKey, value, height, omapKey, false);
  } else {
    rootKey = treeHandler->insert(rootKey, rootPos, omapKey, value, height,
                                  omapKey, false);
  }
  double y;
  if (treeHandler->logTime) {
...
/**
 * This function is used for batch insert which is used at the end of setup
 * phase.
 */
void OMAP::batchInsert(map<Bid, string> pairs) {
  ...
  for (auto pair : pairs) {
    cnt++;
    if (rootKey == 0) {
      rootKey = treeHandler->insert(0, rootPos, pair.first, pair.second, height,
                                    0, false);
    } else {
...
/**
 * This function is used for batch search which is used in the real search
 * procedure
 */
